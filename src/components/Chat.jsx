import { create } from "framer-motion/client";
import React, { useEffect, useState } from "react";
import { useParams } from "react-router-dom";
import { createSocketConnection } from "../utils/socket";
import { useSelector } from "react-redux";
import axios from "axios";
import { BASE_URL } from "../utils/constants";

const Chat = () => {
  const { targetedUserId } = useParams();
  const user = useSelector((store) => store.user);
  const userId = user?._id;
  const firstName = user?.firstName;
  console.log("firstName123", firstName);

  const [message, setMessage] = useState([]);
  console.log("message123", message);
  const [newMessage, setNewMessage] = useState("");


  const fetchChatMessages = async()=>{
    const chat =  await axios.get(BASE_URL+"/chat/"+targetedUserId,{
      withCredentials: true,})
      
      console.log("chat11",chat.data)

      const chatMessages = chat.data.messages.map((msg)=>{

        const {
          senderId, text} = msg
        return {
          firstName: 
          senderId.firstName, 
          lastName: 
          senderId.lastName, 
          text
        }
      })

      setMessage(chatMessages)
  }

  useEffect(() => {
    fetchChatMessages();
  }, []);

  useEffect(() => {
    if(!userId) return
    const socket = createSocketConnection();
    socket.emit("joinChat", {firstName, userId, targetedUserId});


    socket.on("messageReceived", ({firstName, lastName, text})=>{
      console.log("messageReceived", {firstName, text});
      setMessage((prevMessage) => [...prevMessage, {firstName,lastName, text}])

    })
    

    return ()=>{
      socket.disconnect()
    }
  }, [userId, targetedUserId]);

  const sendMessage = () => {
    const socket = createSocketConnection();
    socket.emit("sendMessage", {
      firstName,
      lastName : user?.lastName,
      userId,
      targetedUserId,
      text: newMessage,
    })
    setNewMessage("");

  }

  return (
    <div className="w-3/4 mx-auto border border-gray-600 m-5 h-[70vh] flex flex-col">
      <h1 className="p-5 border-b border-gray-600">Chat</h1>
      {/* display messages */}
      <div className="flex-1 overflow-scroll p-5">
        {message.map((msg, index) => {
          return (
              <div 
              key={index}
              //  className={"chat" + (firstName === msg?.firstName ? "chat-end" : "chat-start")}>
              className={`chat ${firstName === msg?.firstName ? "chat-end" : "chat-start"}`}
>
                <div className="chat-header">
                 {msg?.firstName}
                  <time className="text-xs opacity-50 p-2">2 hours ago</time>
                </div>
                <div className="chat-bubble">{msg.text}</div>
                <div className="chat-footer opacity-50">Seen</div>
              </div>
          );
        })}
      </div>
      <div className="p-5 border-t border-gray-600 flex gap-2">
        <input
        value={newMessage}
          onChange={(e) => setNewMessage(e.target.value)}
          className="flex-1 border border-gray-500 text-white rounded p-2"
          type="text"
          placeholder="Enter message"
        />
        <button onClick={sendMessage} className="btn btn-secondary">Send</button>
      </div>
    </div>
  );
};

export default Chat;
